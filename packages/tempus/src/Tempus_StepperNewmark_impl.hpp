...
template <class Scalar>
void
StepperNewmark<Scalar>::predictVelocity(
...
template <class Scalar>
void
StepperNewmark<Scalar>::predictDisplacement(
...
template <class Scalar>
void
StepperNewmark<Scalar>::correctVelocity(
...
template <class Scalar>
void
StepperNewmark<Scalar>::correctDisplacement(
...
template <class Scalar>
void
StepperNewmark<Scalar>::correctAcceleration(
...
template <class Scalar>
void
StepperNewmark<Scalar>::setModel(
    const Teuchos::RCP<const Thyra::ModelEvaluator<Scalar>>& transientModel) {
#ifdef VERBOSE_DEBUG_OUTPUT
  *out_ << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
#endif
...
template <class Scalar>
void
StepperNewmark<Scalar>::setNonConstModel(
...
template <class Scalar>
void
StepperNewmark<Scalar>::setSolver(std::string solverName) {
#ifdef VERBOSE_DEBUG_OUTPUT
  *out_ << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
#endif
  using Teuchos::RCP;
...
template <class Scalar>
void
StepperNewmark<Scalar>::setSolver(
...
template <class Scalar>
void
StepperNewmark<Scalar>::initialize() {
#ifdef VERBOSE_DEBUG_OUTPUT
  *out_ << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
#endif
  this->setSolver();
...
template <class Scalar>
void
StepperNewmark<Scalar>::takeStep(
...
StepperNewmark<Scalar>::getDefaultStepperState() {
#ifdef VERBOSE_DEBUG_OUTPUT
  *out_ << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
#endif
...
template <class Scalar>
std::string
StepperNewmark<Scalar>::description() const {
#ifdef VERBOSE_DEBUG_OUTPUT
  *out_ << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
#endif
...
template <class Scalar>
void
StepperNewmark<Scalar>::describe(
...
template <class Scalar>
void
StepperNewmark<Scalar>::setParameterList(
  ...
  *out_ << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      stepperType != "Newmark Beta", std::logic_error,
  ...
  if (stepperPL_->isSublist("Newmark Beta Parameters")) {
    ...
    if (scheme_name == "Not Specified") {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
          ...
          "\nError in 'Newmark' stepper: invalid value of Beta = "
              << beta_ << ".  Please select Beta >= 0 and <= 1. \n");
      TEUCHOS_TEST_FOR_EXCEPTION(
          (gamma_ > 1.0) || (gamma_ < 0.0), std::logic_error,
          "\nError in 'Newmark' stepper: invalid value of Gamma = "
      ...
      *out_ << "\nScheme Name = " << scheme_name << ".  Using values \n"
      ...
      if (scheme_name == "Average Acceleration") {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(
            ...
            "\nError in Tempus::StepperNewmark!  Invalid Scheme Name = "
                << scheme_name << ".  \n"
    ...
    if (beta_ == 0.0) {
      *out_ << "\nWARNING: Running (implicit implementation of) Newmark Beta "
               "Stepper with Beta = 0.0, which \n"
            << "specifies an explicit scheme.  Mass lumping is not possible, "
               "so this will be slow!  To run explicit \n"
            << "implementation of Newmark Beta Stepper, please re-run with "
               "'Stepper Type' = 'Newmark Beta Explicit'.\n"
            << "This stepper allows for mass lumping when called through "
               "Piro::TempusSolver.\n";
    }
  } else {
    *out_ << "\nNo Newmark Beta Parameters sublist found in input file; using "
