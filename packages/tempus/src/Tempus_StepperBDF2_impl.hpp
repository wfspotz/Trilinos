  ...
  const Teuchos::RCP<SolutionHistory<Scalar> >& solutionHistory)
{
  ...
  TEMPUS_FUNC_TIME_MONITOR("Tempus::StepperBDF2::takeStep()");
  {
    ...
    Teuchos::RCP<TimeDerivative<Scalar> > timeDer =
      Teuchos::rcp(new StepperBDF2TimeDerivative<Scalar>(1.0/dt,xOld));
   
    //Set up BDF2 Time Derivative 
    //IKT, FIXME: replace above code with the following when ready  
    Teuchos::RCP<TimeDerivative<Scalar> > timeDerBDF2 =
