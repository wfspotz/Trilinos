...
class SecondOrderResidualModelEvaluator
 ...
 public:
    ...
    *out_ << "    schemeName = " << schemeName << "\n";
    ...
    if (schemeName == "Newmark Beta" || schemeName == "HHT-Alpha") {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
          ...
          "Error: SecondOrderResidualModelEvaluator called with unsopported "
          "schemeName = "
              << schemeName << "!  Supported schemeNames are: 'Newmark Beta' "
                               "and 'HHT-Alpha'.\n");
    }
  }
  ...
  void
  initializeNewmark(
      Teuchos::RCP<Vector> a, Teuchos::RCP<Vector> v_pred,
      Teuchos::RCP<Vector> d_pred, Scalar delta_t, Scalar t, Scalar beta,
      Scalar gamma) {
#ifdef VERBOSE_DEBUG_OUTPUT
    *out_ << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
#endif
  ...
  getNominalValues() const {
    ...
    *out_ << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
#endif
    Thyra::ModelEvaluatorBase::InArgs<Scalar> nominalValues =
        transientModel_->getNominalValues();
